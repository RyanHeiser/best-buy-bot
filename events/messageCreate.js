...
module.exports = {
	...
	async execute(message) {
        ...
        const prompt = message.content.replace("<@" + process.env.BOT_ID + ">", "Best Buy Satan"); // remove the bot's id from prompt

        if (message.author.bot) {
            if (message.author.id == process.env.IKEA_ID) {
                if (!message.mentions.has(process.env.BOT_ID)) {
                    promptList[promptList.length] = {role: "user", content: message.author.displayName + ": " + prompt}; // add Ikea God's message to the prompt list
                }
                // random chance to not respond to Ikea God
        ...
        if (message.mentions.has(process.env.BOT_ID)) {
            ...
            if (responded == false) {
                message.reply("DO NOT DARE DISTURB MY RESPONSE. WAIT FOR ME TO FINISH.");
                if (message.member.voice.channel && tts) {
                    speechSynth("DO NOT DARE DISTURB MY RESPONSE. WAIT FOR ME TO FINISH.").then(result => {
            ...
            await openAI.generateText(promptList, model, 400) // request chat completion
                .then(text => {
                    ...
                    if (reply) {
                        var discordResponse = text.replace("Ikea God", "<@" + process.env.IKEA_ID + ">");
                        if (message.author.bot) {
